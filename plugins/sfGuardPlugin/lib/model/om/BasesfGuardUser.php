...
abstract class BasesfGuardUser extends BaseObject  implements Persistent
{
  ...
  const PEER = 'sfGuardUserPeer';
	 ...
	 */
	protected $collMensajesRelatedByDestinatarioId;

	/**
	 * @var        array Mensaje[] Collection to store aggregation of Mensaje objects.
	 */
	protected $collMensajesRelatedByRemitenteId;
	...
	/**
	 * @var        array RedSocialXPerfil[] Collection to store aggregation of RedSocialXPerfil objects.
	 */
	protected $collRedSocialXPerfils;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->collMensajesRelatedByDestinatarioId = null;

			$this->collMensajesRelatedByRemitenteId = null;

			$this->collPosts = null;

			$this->collComentarios = null;

			$this->collRedSocialXPerfils = null;
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collMensajesRelatedByDestinatarioId !== null) {
				foreach ($this->collMensajesRelatedByDestinatarioId as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collMensajesRelatedByRemitenteId !== null) {
				foreach ($this->collMensajesRelatedByRemitenteId as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
			...
			if ($this->collRedSocialXPerfils !== null) {
				foreach ($this->collRedSocialXPerfils as $referrerFK) {
					if (!$referrerFK->isDeleted()) {
						$affectedRows += $referrerFK->save($con);
					}
				}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach ($this->getMensajesRelatedByDestinatarioId() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addMensajeRelatedByDestinatarioId($relObj->copy($deepCopy));
				}
			}

			foreach ($this->getMensajesRelatedByRemitenteId() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addMensajeRelatedByRemitenteId($relObj->copy($deepCopy));
				}
			}
			...
			foreach ($this->getRedSocialXPerfils() as $relObj) {
				if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
					$copyObj->addRedSocialXPerfil($relObj->copy($deepCopy));
				}
			}
	...
	/**
	 * Clears out the collMensajesRelatedByDestinatarioId collection
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addMensajesRelatedByDestinatarioId()
	 */
	public function clearMensajesRelatedByDestinatarioId()
	{
		$this->collMensajesRelatedByDestinatarioId = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collMensajesRelatedByDestinatarioId collection.
	 *
	 * By default this just sets the collMensajesRelatedByDestinatarioId collection to an empty array (like clearcollMensajesRelatedByDestinatarioId());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initMensajesRelatedByDestinatarioId()
	{
		$this->collMensajesRelatedByDestinatarioId = new PropelObjectCollection();
		$this->collMensajesRelatedByDestinatarioId->setModel('Mensaje');
	}
	 ...
	 * @throws     PropelException
	 */
	public function getMensajesRelatedByDestinatarioId($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collMensajesRelatedByDestinatarioId || null !== $criteria) {
			if ($this->isNew() && null === $this->collMensajesRelatedByDestinatarioId) {
				// return empty collection
				$this->initMensajesRelatedByDestinatarioId();
			} else {
				$collMensajesRelatedByDestinatarioId = MensajeQuery::create(null, $criteria)
					->filterBysfGuardUserRelatedByDestinatarioId($this)
					->find($con);
				if (null !== $criteria) {
					return $collMensajesRelatedByDestinatarioId;
				}
				$this->collMensajesRelatedByDestinatarioId = $collMensajesRelatedByDestinatarioId;
			}
		}
		return $this->collMensajesRelatedByDestinatarioId;
	}
	 ...
	 * @throws     PropelException
	 */
	public function countMensajesRelatedByDestinatarioId(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collMensajesRelatedByDestinatarioId || null !== $criteria) {
			if ($this->isNew() && null === $this->collMensajesRelatedByDestinatarioId) {
				return 0;
			...
			return count($this->collMensajesRelatedByDestinatarioId);
		}
	}
	 ...
	 * @throws     PropelException
	 */
	public function addMensajeRelatedByDestinatarioId(Mensaje $l)
	{
		if ($this->collMensajesRelatedByDestinatarioId === null) {
			$this->initMensajesRelatedByDestinatarioId();
		}
		if (!$this->collMensajesRelatedByDestinatarioId->contains($l)) { // only add it if the **same** object is not already associated
			$this->collMensajesRelatedByDestinatarioId[]= $l;
			$l->setsfGuardUserRelatedByDestinatarioId($this);
		}
	}

	/**
	 * Clears out the collMensajesRelatedByRemitenteId collection
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addMensajesRelatedByRemitenteId()
	 */
	public function clearMensajesRelatedByRemitenteId()
	{
		$this->collMensajesRelatedByRemitenteId = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collMensajesRelatedByRemitenteId collection.
	 *
	 * By default this just sets the collMensajesRelatedByRemitenteId collection to an empty array (like clearcollMensajesRelatedByRemitenteId());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initMensajesRelatedByRemitenteId()
	{
		$this->collMensajesRelatedByRemitenteId = new PropelObjectCollection();
		$this->collMensajesRelatedByRemitenteId->setModel('Mensaje');
	}
	 ...
	 * @throws     PropelException
	 */
	public function getMensajesRelatedByRemitenteId($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collMensajesRelatedByRemitenteId || null !== $criteria) {
			if ($this->isNew() && null === $this->collMensajesRelatedByRemitenteId) {
				// return empty collection
				$this->initMensajesRelatedByRemitenteId();
			} else {
				$collMensajesRelatedByRemitenteId = MensajeQuery::create(null, $criteria)
					->filterBysfGuardUserRelatedByRemitenteId($this)
					->find($con);
				if (null !== $criteria) {
					return $collMensajesRelatedByRemitenteId;
				}
				$this->collMensajesRelatedByRemitenteId = $collMensajesRelatedByRemitenteId;
			}
		}
		return $this->collMensajesRelatedByRemitenteId;
	}
	 ...
	 * @throws     PropelException
	 */
	public function countMensajesRelatedByRemitenteId(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collMensajesRelatedByRemitenteId || null !== $criteria) {
			if ($this->isNew() && null === $this->collMensajesRelatedByRemitenteId) {
				return 0;
			...
			return count($this->collMensajesRelatedByRemitenteId);
		}
	}
	 ...
	 * @throws     PropelException
	 */
	public function addMensajeRelatedByRemitenteId(Mensaje $l)
	{
		if ($this->collMensajesRelatedByRemitenteId === null) {
			$this->initMensajesRelatedByRemitenteId();
		}
		if (!$this->collMensajesRelatedByRemitenteId->contains($l)) { // only add it if the **same** object is not already associated
			$this->collMensajesRelatedByRemitenteId[]= $l;
			$l->setsfGuardUserRelatedByRemitenteId($this);
		}
	}
	 ...
	 * @return     PropelCollection|array Post[] List of Post objects
	 */
	public function getPostsJoinTipoPost($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = PostQuery::create(null, $criteria);
		$query->joinWith('TipoPost', $join_behavior);
	 ...
	 * @return     PropelCollection|array Comentario[] List of Comentario objects
	 */
	public function getComentariosJoinPost($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = ComentarioQuery::create(null, $criteria);
	...
	/**
	 * Clears out the collRedSocialXPerfils collection
	 *
	 * This does not modify the database; however, it will remove any associated objects, causing
	 * them to be refetched by subsequent calls to accessor method.
	 *
	 * @return     void
	 * @see        addRedSocialXPerfils()
	 */
	public function clearRedSocialXPerfils()
	{
		$this->collRedSocialXPerfils = null; // important to set this to NULL since that means it is uninitialized
	}

	/**
	 * Initializes the collRedSocialXPerfils collection.
	 *
	 * By default this just sets the collRedSocialXPerfils collection to an empty array (like clearcollRedSocialXPerfils());
	 * however, you may wish to override this method in your stub class to provide setting appropriate
	 * to your application -- for example, setting the initial array to the values stored in database.
	 *
	 * @return     void
	 */
	public function initRedSocialXPerfils()
	{
		$this->collRedSocialXPerfils = new PropelObjectCollection();
		$this->collRedSocialXPerfils->setModel('RedSocialXPerfil');
	}

	/**
	 * Gets an array of RedSocialXPerfil objects which contain a foreign key that references this object.
	 *
	 * If the $criteria is not null, it is used to always fetch the results from the database.
	 * Otherwise the results are fetched from the database the first time, then cached.
	 * Next time the same method is called without $criteria, the cached collection is returned.
	 ...
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 * @return     PropelCollection|array RedSocialXPerfil[] List of RedSocialXPerfil objects
	 * @throws     PropelException
	 */
	public function getRedSocialXPerfils($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collRedSocialXPerfils || null !== $criteria) {
			if ($this->isNew() && null === $this->collRedSocialXPerfils) {
				// return empty collection
				$this->initRedSocialXPerfils();
			} else {
				$collRedSocialXPerfils = RedSocialXPerfilQuery::create(null, $criteria)
					->filterBysfGuardUser($this)
					->find($con);
				if (null !== $criteria) {
					return $collRedSocialXPerfils;
				}
				$this->collRedSocialXPerfils = $collRedSocialXPerfils;
			}
		}
		return $this->collRedSocialXPerfils;
	}

	/**
	 * Returns the number of related RedSocialXPerfil objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related RedSocialXPerfil objects.
	 * @throws     PropelException
	 */
	public function countRedSocialXPerfils(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collRedSocialXPerfils || null !== $criteria) {
			if ($this->isNew() && null === $this->collRedSocialXPerfils) {
				return 0;
			} else {
				$query = RedSocialXPerfilQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
			...
			return count($this->collRedSocialXPerfils);
		}
	}

	/**
	 * Method called to associate a RedSocialXPerfil object to this object
	 * through the RedSocialXPerfil foreign key attribute.
	 *
	 * @param      RedSocialXPerfil $l RedSocialXPerfil
	 * @return     void
	 * @throws     PropelException
	 */
	public function addRedSocialXPerfil(RedSocialXPerfil $l)
	{
		if ($this->collRedSocialXPerfils === null) {
			$this->initRedSocialXPerfils();
		}
		if (!$this->collRedSocialXPerfils->contains($l)) { // only add it if the **same** object is not already associated
			$this->collRedSocialXPerfils[]= $l;
			$l->setsfGuardUser($this);
		}
	}
	 ...
	 * Otherwise if this sfGuardUser is new, it will return
	 * an empty collection; or if this sfGuardUser has previously
	 * been saved, it will retrieve related RedSocialXPerfils from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in sfGuardUser.
	 ...
	 * @return     PropelCollection|array RedSocialXPerfil[] List of RedSocialXPerfil objects
	 */
	public function getRedSocialXPerfilsJoinRedSocial($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = RedSocialXPerfilQuery::create(null, $criteria);
		$query->joinWith('RedSocial', $join_behavior);

		return $this->getRedSocialXPerfils($query, $con);
	}
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			...
			if ($this->collMensajesRelatedByDestinatarioId) {
				foreach ((array) $this->collMensajesRelatedByDestinatarioId as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collMensajesRelatedByRemitenteId) {
				foreach ((array) $this->collMensajesRelatedByRemitenteId as $o) {
					$o->clearAllReferences($deep);
				}
			}
			if ($this->collPosts) {
			...
			if ($this->collRedSocialXPerfils) {
				foreach ((array) $this->collRedSocialXPerfils as $o) {
					$o->clearAllReferences($deep);
				}
			}
		} // if ($deep)
